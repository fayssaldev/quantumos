...
static uint8_t Flash_WriteHalfWord_Buf(uint16_t WriteAddr, uint16_t *pBuffer, uint16_t NumToWrite)
{
    ...
    endaddr = writeAddr + (uint32_t)NumToWrite * sizeof(uint16_t); //д�������ַ
    if(addrx < 0X1FFF0000)          //�Ƿ�Ϊ���洢��
    {
        while(addrx < endaddr)
        {
            if(Flash_ReadHalfWord(addrx) != 0XFFFF) //�Է�0xFFFF �ĵط����Ȳ���
            {
                status = FLASH_EraseSector(GetSector(addrx), VoltageRange_3); //VCC=2.7~3.6V֮�䣡��
