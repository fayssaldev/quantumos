...
byte DS3231::getHour(bool& h12, bool& PM) {
	...
	h12 = (temp_buffer & 0x40);//B01000000
	if (h12) {
		PM = temp_buffer & 0x20;//B00100000
		hour = bcdToDec(temp_buffer & 0x1F);//B00011111
	} else {
		hour = bcdToDec(temp_buffer & 0x3F);//B00111111
	}
	return hour;
}
...
byte DS3231::getMonth(bool& Century) {
	...
	Wire.requestFrom(CLOCK_ADDRESS, 1);
	temp_buffer = Wire.read();
	Century = temp_buffer & 0x80;//B10000000
	return (bcdToDec(temp_buffer & 0x7F)) ;//B01111111
}
...
void DS3231::setSecond(byte Second) {
	...
	writeControlByte((temp_buffer & 0x7F), 1);//B01111111
}
...
void DS3231::setHour(byte Hour) {
	...
	Wire.endTransmission();
	Wire.requestFrom(CLOCK_ADDRESS, 1);
	h12 = (Wire.read() & 0x40);//B01000000
	// if h12 is true, it's 12h mode; false is 24h.

	if (h12) {
		// 12 hour
		if (Hour > 12) {
			Hour = decToBcd(Hour-12) | 0x60;//B01100000
		} else {
			Hour = decToBcd(Hour) & 0xDF;//B11011111
		}
	} else {
		// 24 hour
		Hour = decToBcd(Hour) & 0XBF;//B10111111
	}
...
void DS3231::setClockMode(bool h12) {
	...
	if (h12) {
		temp_buffer = temp_buffer | 0X40;//B01000000
	} else {
		temp_buffer = temp_buffer & 0xBF;//B10111111
	}
...
void DS3231::getA1Time(byte& A1Day, byte& A1Hour, byte& A1Minute, byte& A1Second, byte& AlarmBits, bool& A1Dy, bool& A1h12, bool& A1PM) {
	...
	A1Second	= bcdToDec(temp_buffer & 0x7F);//B01111111
	// put A1M1 bit in position 0 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & 0x80)>>7;//B10000000

	temp_buffer		= Wire.read();	// Get A1M2 and A1 minutes
	A1Minute	= bcdToDec(temp_buffer & 0x7F);//B01111111
	// put A1M2 bit in position 1 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & 0x80)>>6;//B10000000

	temp_buffer	= Wire.read();	// Get A1M3 and A1 Hour
	// put A1M3 bit in position 2 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & 0x80)>>5;//B10000000
	// determine A1 12/24 mode
	A1h12		= temp_buffer & 0x40;//B01000000
	if (A1h12) {
		A1PM	= temp_buffer & 0x20;//B00100000			// determine am/pm
		A1Hour	= bcdToDec(temp_buffer & 0x1F);//B00011111	// 12-hour
	} else {
		A1Hour	= bcdToDec(temp_buffer & 0x3F);//B00111111	// 24-hour
	}

	temp_buffer	= Wire.read();	// Get A1M4 and A1 Day/Date
	// put A1M3 bit in position 3 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & 0x80)>>4;//B10000000
	// determine A1 day or date flag
	A1Dy		= (temp_buffer & 0x40)>>6;//B01000000
	if (A1Dy) 
	{
		// alarm is by day of week, not date.
		A1Day	= bcdToDec(temp_buffer & B00001111);
	} 
	else 
	{
		// alarm is by date, not day of week.
		A1Day	= bcdToDec(temp_buffer & B00111111);
	}
}
...
void DS3231::getA2Time(byte& A2Day, byte& A2Hour, byte& A2Minute, byte& AlarmBits, bool& A2Dy, bool& A2h12, bool& A2PM) {
	...
	A2Minute	= bcdToDec(temp_buffer & 0x7F);//B01111111
	// put A2M2 bit in position 4 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & B10000000)>>3;

	temp_buffer	= Wire.read();	// Get A2M3 and A2 Hour
	// put A2M3 bit in position 5 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & B10000000)>>2;
	// determine A2 12/24 mode
	A2h12		= (temp_buffer & B01000000);
	if (A2h12) {
		A2PM	= temp_buffer & B00100000;			// determine am/pm
		A2Hour	= bcdToDec(temp_buffer & B00011111);	// 12-hour
	} else {
		A2Hour	= bcdToDec(temp_buffer & B00111111);	// 24-hour
	}

	temp_buffer	= Wire.read();	// Get A2M4 and A1 Day/Date
	// put A2M4 bit in position 6 of DS3231_AlarmBits.
	AlarmBits	= AlarmBits | (temp_buffer & B10000000)>>1;
	// determine A2 day or date flag
	A2Dy		= (temp_buffer & B01000000)>>6;
	if (A2Dy) {
		// alarm is by day of week, not date.
		A2Day	= bcdToDec(temp_buffer & B00001111);
	} else {
		// alarm is by date, not day of week.
		A2Day	= bcdToDec(temp_buffer & B00111111);
	}
}
...
void DS3231::setA1Time(byte A1Day, byte A1Hour, byte A1Minute, byte A1Second, byte AlarmBits, bool A1Dy, bool A1h12, bool A1PM) {
	...
	Wire.write(decToBcd(A1Second) | ((AlarmBits & B00000001) << 7));
	// Send A1 Minute and A1M2
	Wire.write(decToBcd(A1Minute) | ((AlarmBits & B00000010) << 6));
	// Figure out A1 hour 
	if (A1h12) {
		...
		if (A1PM) {
			...
			temp_buffer = decToBcd(A1Hour) | B01100000;
		} else {
			// Morning
			// Convert the hour to BCD and add appropriate flags.
			temp_buffer = decToBcd(A1Hour) | B01000000;
		}
	} else {
		// Now for 24h
		temp_buffer = decToBcd(A1Hour); 
	}
	temp_buffer = temp_buffer | ((AlarmBits & B00000100)<<5);
	// A1 hour is figured out, send it
	Wire.write(temp_buffer); 
	// Figure out A1 day/date and A1M4
	temp_buffer = ((AlarmBits & B00001000)<<4) | decToBcd(A1Day);
	if (A1Dy) {
		// Set A1 Day/Date flag (Otherwise it's zero)
		temp_buffer = temp_buffer | B01000000;
	}
	Wire.write(temp_buffer);
	// All done!
	Wire.endTransmission();
...
void DS3231::setA2Time(byte A2Day, byte A2Hour, byte A2Minute, byte AlarmBits, bool A2Dy, bool A2h12, bool A2PM) {
	...
	byte temp_buffer;
	Wire.beginTransmission(CLOCK_ADDRESS);
	Wire.write(0xB);	// A1 starts at Bh
	// Send A2 Minute and A2M2
	Wire.write(decToBcd(A2Minute) | ((AlarmBits & B00010000) << 3));
	// Figure out A2 hour 
	if (A2h12) {
		// Start by converting existing time to h12 if it was given in 24h.
		if (A2Hour > 12) {
		...
		if (A2PM) {
			// Afternoon
			// Convert the hour to BCD and add appropriate flags.
			temp_buffer = decToBcd(A2Hour) | B01100000;
		} else {
			// Morning
			// Convert the hour to BCD and add appropriate flags.
			temp_buffer = decToBcd(A2Hour) | B01000000;
		}
	} else {
		// Now for 24h
	...
	// add in A2M3 bit
	temp_buffer = temp_buffer | ((AlarmBits & B00100000)<<2);
	// A2 hour is figured out, send it
	Wire.write(temp_buffer); 
	// Figure out A2 day/date and A2M4
	temp_buffer = ((AlarmBits & B01000000)<<1) | decToBcd(A2Day);
	if (A2Dy) {
...
void DS3231::turnOnAlarm(byte Alarm) {
	...
	// modify control byte
	if (Alarm == 1) {
		temp_buffer = temp_buffer | B00000101;
	} else {
		temp_buffer = temp_buffer | B00000110;
	}
...
void DS3231::turnOffAlarm(byte Alarm) {
	...
	// modify control byte
	if (Alarm == 1) {
		temp_buffer = temp_buffer & B11111110;
	} else {
		temp_buffer = temp_buffer & B11111101;
	}
	writeControlByte(temp_buffer, 0);
}
...
bool DS3231::checkAlarmEnabled(byte Alarm) {
	...
	byte temp_buffer = readControlByte(0);
	if (Alarm == 1) {
		result = temp_buffer & B00000001;
	} else {
		result = temp_buffer & B00000010;
	}
	return result;
}
...
void DS3231::enableOscillator(bool TF, bool battery, byte frequency) {
	...
	byte temp_buffer;
	temp_buffer = readControlByte(0) & B11100111;
	if (battery) {
		// turn on BBSQW flag
		temp_buffer = temp_buffer | B01000000;
	} else {
		// turn off BBSQW flag
		temp_buffer = temp_buffer & B10111111;
	}
	if (TF) {
		// set ~EOSC to 0 and INTCN to zero.
		temp_buffer = temp_buffer & B01111011;
	} else {
		// set ~EOSC to 1, leave INTCN as is.
		temp_buffer = temp_buffer | B10000000;
	}
...
void DS3231::enable32kHz(bool TF) {
	...
	if (TF) {
		// turn on 32kHz pin
		temp_buffer = temp_buffer | B00001000;
	} else {
		// turn off 32kHz pin
		temp_buffer = temp_buffer & B11110111;
	}
	writeControlByte(temp_buffer, 1);
}
